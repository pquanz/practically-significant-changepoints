...
$$
\begin{aligned}
    T_1 (\lambda) & = \bigg( X_1^\top (0, ..., 0, X_{m+2}, ..., X_{\lfloor \lambda k \rfloor}) + \cdots + X_{\lfloor \lambda k \rfloor}^\top (X_1, ..., X_{\lfloor \lambda k \rfloor - m - 1}, 0, ..., 0) \bigg) \begin{pmatrix}
        1 \\ \vdots \\ 1
      \end{pmatrix}\\
...
$$
\hat S_n = \{ \ell = 1, \ldots, p \mid \hat \delta_\ell^2 > \hat v_\ell \cdot \log^{3/2} (p) \}.
$$

The computation can be long and it is recommended to call this function with `parallelize = TRUE`.




## Contents of the file changepoint.R
